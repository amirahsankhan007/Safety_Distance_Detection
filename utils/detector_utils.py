...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np):
    ...
    for i in range(num_hands_detect):
        if (scores[i] > score_thresh):
            ...
            bounding_mid=(int((p1[0]+p2[0])/2),int(top))
            
            return bounding_mid


# Show fps value on image.
def draw_text_on_image1(fps, image_np):
    cv2.putText(image_np, fps, (20, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.75, (77, 255, 9), 2)
    
def draw_text_on_image2(dist, image_np):
    cv2.putText(image_np, dist, (450, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.75, (77, 255, 9), 2) 
    
def draw_text_on_image3(alert, image_np):
    cv2.putText(image_np, alert, (300, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0,0), 2) 
    cv2.rectangle(image_np, (280,20), (395,60), (255,0,0), thickness=3, lineType=8, shift=0)     
